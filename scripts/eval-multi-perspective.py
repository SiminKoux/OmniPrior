...
def save_stats(train_metrics: dict, test_metrics: dict, output_dir: str = "./logs"):
    ...
    overall_stats = {
        split: {
            ...
            for metric in ["psnr", "msssim", "lpips", "dyn_psnr", "dyn_ssim", "bbox_lpips"]
        }
...
class ResultEvaluator:
    ...
    def _load_dynamic_masks(self) -> Dict[str, Dict[int, Dict[str, torch.Tensor]]]:
        ...
        """
        mask_dir = os.path.join(self.data_dir, "transformed_masks")
        ...
        for lens_id in range(1, 7):  # lens01 to lens06
            mask_file = os.path.join(mask_dir, f"crop{lens_id:02d}.npz")
            if os.path.exists(mask_file):
                mask_npz[lens_id] = dict(np.load(mask_file))
                print(f"  Loaded crop{lens_id:02d}.npz with {len(mask_npz[lens_id])} frames")
    ...
    def evaluate_images(self, save_dir: Optional[str] = None):
        ...
        for split in ['train', 'test']:
            ...
            gt_dir = Path(os.path.join(split_dir, "gt_final_pers"))
            if self.model_name == "omniprior":
                render_dir = Path(os.path.join(split_dir, f"renders_final_pers"))
            else:
                render_dir = Path(os.path.join(split_dir, f"renders_final_pers_{self.model_name}"))
                    ...
                    if dyn_mask is not None:
                        ...
                        if valid_px > 0:
                            ...
                            if len(bboxes) > 0:
                                ...
                                valid_bbox_areas = []
                                patch_save_dir = os.path.join(split_dir, f"bbox_{self.model_name}", f"crop0{lens_id}")
                                ...
                                for i, (x1, y1, x2, y2) in enumerate(bboxes):
                                    ...
                                    try:
                                        ...
                                        ps = self.psnr(pred_patch, gt_patch).item()
                                        sm = self.ssim(pred_patch, gt_patch).item()
    ...
    def _save_results(self, results: Dict, frame_metrics: Dict, save_dir: str):
        ...
        for split in ['train', 'test']:
            ...
            for lens_id, lens_metrics in results[split].items():
                all_results["per_lens"][split][f"crop{lens_id:02d}"] = {
                    metric: float(np.mean(values))
                    for metric, values in lens_metrics.items()
                }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='omniprior', 
                        choices=['omniprior', 'degauss', 'swift4d'], help='Model name to evaluate')
    ...
    output_dir = f"{result_root}/stats/multi-perspective/{model_name}"
