...
# Motion-Freeze Rendering Modes:
# - Static Freeze Mode (SFM):  --freeze_mode static  (fixed camera, animated scene)
# - Dynamic Freeze Mode (DFM): --freeze_mode dynamic (moving camera, frozen scene)
# Note: Adjust --dyn_app_threshold based on scene (paper setting: 0.7 for 'loft')

for SCENE in $SCENE_LIST; do
    echo "Running $SCENE, MCMCStrategy"
    RESULT_DIR="./results/$SCENE/init_colmap_metric"
    CKPT_PATH="$RESULT_DIR/ckpts/ckpt_final_rank0.pt"
    CUDA_VISIBLE_DEVICES=0 python runner/dyn_apps.py mcmc \
        --data_factor 1 \
